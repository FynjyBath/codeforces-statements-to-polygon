...
Утилита для переноса задач из HTML страницы Codeforces в Polygon. Скрипт парсит условия, ограничения, примеры и создаёт новые задачи в Polygon с префиксом, который задаётся в командной строке (к нему автоматически добавляются суффиксы `-a`, `-b`, ...).
